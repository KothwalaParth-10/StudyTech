...
export function getPasswordResetToken(email, setEmailSent) {
  return async (dispatch) => {
    ...
    try {
      ...
      console.log("RESETPASSWORD TOKEN API ERROR............", error)
...
export function uploadDp(file, token, user) {
  return async (dispatch) => { 
    if (!file) {
    ...
    const toast_id = toast.loading("Uploading profile picture...");
    try {
      ...
      toast.success(response.data.message);
      // dispatch(setUser({...user,response.updated_user}))
      const updatedUser = { ...user, ...response?.data?.updated_user };
      console.log(response?.data?.updated_user);
      dispatch(setUser(updatedUser));
      localStorage.setItem("user", JSON.stringify(updatedUser));
    } catch (error) {
      console.log("Profile Picure Upload Error", error);
      toast.dismiss(toast_id);
